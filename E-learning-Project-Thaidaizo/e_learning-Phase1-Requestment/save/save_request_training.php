                ...
                if ($inputAttach == 0) {
                    foreach ($Trainees as $InputTrainees) {
                        ...
                        $SaveOneParams = array(
                            ...
                            $Authen, $inputCourse, $SendInfo, 1
                        );
                        $SaveOneStmt = sqlsrv_query($connRequest, $SaveOneSql, $SaveOneParams);
                    }
                    if ($SaveOneStmt == true) {
                        // แบบส่งเมลได้จริงๆแต่ต้อง เน็ต บริษัท
                                    ...
                                    if (move_uploaded_file($_FILES['inputImgFile']["tmp_name"], "../assets/img/request/" . $KeyID . $KeyTime . $_FILES['inputImgFile']['name'])) {
                                        ...
                                        foreach ($Trainees as $InputTrainees) {
                                            ...
                                            $SaveOneParams = array(
                                                $KeyID, 4, $InputTrainees, $FromPeriod, $ToPeriod,
                                                $inputDurationDay, $inputDurationTime, $inputCost, $SesUserID,
                                                $Authen, $inputCourse, $SendInfo, $ImgNew, 1
                                            );
                                            $SaveOneStmt = sqlsrv_query($connRequest, $SaveOneSql, $SaveOneParams);
                                        }
                                        if ($SaveOneStmt == true) {
                                    ...
                                    if (move_uploaded_file($_FILES['inputPdfFile']["tmp_name"], "../assets/img/request/" . $KeyID . $KeyTime . $_FILES['inputPdfFile']['name'])) {
                                        ...
                                        foreach ($Trainees as $InputTrainees) {
                                            ...
                                            $SaveOneParams = array(
                                                $KeyID, 4, $InputTrainees, $FromPeriod, $ToPeriod,
                                                $inputDurationDay, $inputDurationTime, $inputCost, $SesUserID,
                                                $Authen, $inputCourse, $SendInfo, $PDFNew, 1
                                            );
                                            $SaveOneStmt = sqlsrv_query($connRequest, $SaveOneSql, $SaveOneParams);
                                        }
                                        if ($SaveOneStmt == true) {
