...
for($i=0;$i<count($_POST["CheckStaff"]);$i++) {
    ...
    $Queryer11 = "SELECT ReqDate,TrnTime,ReqDay,ReqHour,ReqSumTime,ReqRemark,ReqChecker FROM ReqInfo 
	WHERE (ReqNo='$ItemID') 
	...
	$objResulty11 = sqlsrv_fetch_array($objQueryer11,SQLSRV_FETCH_ASSOC);
		...
		WHERE (ReqNo='$ItemID') 
		AND (ReqChecker='".$objResulty11["ReqChecker"]."') 
		AND (ReqApprover='$SesUserID') 
		AND (ReqApproveDate IS NOT NULL)
		AND (ReqType = 4) 
		AND (Status = 0)";
	...
	WHERE (EmpUserID='".$objResulty11["ReqChecker"]."')";
	...
	$strSubject = "Reject a request to check (from Verify).";
...
$strMessage .= "</table>
	...
	$flgSendCheck = @mail($strTo,$strSubject,$strMessage,$strHeader);  // @ = No Show Error // #3D5AFE
}
	if($flgSendCheck){
		//echo "Email Sending.";
		...
		echo "<script type=text/javascript>alert('Error!! Email Can Not Send to Check.');window.top.window.TrainingResult('0');</script>";
		exit();
    }
