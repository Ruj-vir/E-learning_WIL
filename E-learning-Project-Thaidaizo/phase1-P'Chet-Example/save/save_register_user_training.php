<?php
  ...
  if(isset($_POST["BttSubmitRegister"]) == '1') {
    if(trim($_POST["inputItemKey"]) != NULL) {
        ...
        $CheckFileSQL = "SELECT COUNT(DISTINCT TrainRecNo) AS NumCheckFile,PicturePath,PostReport1,PostReport2,PostReport3 FROM TrainRecDtl 
        WHERE (TrainRecNo = '$inputItemKey') 
        AND (EmployeeID = '$SesUserID') 
        AND (TrainDtlStatus >= 1) 
        GROUP BY PicturePath,PostReport1,PostReport2,PostReport3";
        ...
        $PostReport1 = $CheckFileResult["PostReport1"];
        $PostReport2 = $CheckFileResult["PostReport2"];
        $PostReport3 = $CheckFileResult["PostReport3"];
        ...
        if($NumCheckFile > 0) {
          ...
          if($NumAllOpen > 0) {
              ...
              $SaveOneSql = "INSERT INTO TrainRecDtl (TrainRecNo, EmployeeID, TrainRecDate, PicturePath,PostReport1,PostReport2,PostReport3, TrainDtlStatus, CreateBy, CreateDate) 
              VALUES (?, ?, DATEADD(DAY,0,DATEDIFF(DAY,0,GETDATE())), ?, ?, ?, ?, ?, ?, GETDATE())";
              $SaveOneParams = array($inputItemKey, $SesUserID, $PicturePath, $PostReport1, $PostReport2, $PostReport3, 1, $SesUserID);
