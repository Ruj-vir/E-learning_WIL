                            ...
                            ?>
                                    <tr class="tr">
                                        ...
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            <button type="button" class="btn btn-sm btn-outline-dark BttAttachFile" title="Attach file" data-id="<?php echo $ItemReqNo; ?>" data-emp="<?php echo $ClosingResult["EmpUserID"]; ?>"><i class="fas fa-paperclip"></i></button>
                                            <button type="button" class="btn btn-sm btn-outline-dark BttAttachTxT" title="Attach post" data-id="<?php echo $ItemReqNo; ?>" data-emp="<?php echo $ClosingResult["EmpUserID"]; ?>"><i class="fas fa-file-alt"></i></button>
                                        </div>
                                        </td>
                                        <td>
...
<div class="modal fade" id="ModalTraineesList" tabindex="-1" role="dialog" aria-labelledby="reque_title" aria-hidden="true">
  ...
  <!-- Modal Post -->
  <div class="modal fade" id="AttachmentTxTModal" tabindex="-1" role="dialog" aria-labelledby="AttachmenTxTTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header text-greenblue">
          <h5 class="modal-title" id="AttachmenTxTTitle">Post</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
            ...
            <input type="hidden" id="ItemReqID" name="ItemReqID" readonly>
            <input type="hidden" id="ItemEmpID" name="ItemEmpID" readonly>
            <div id="post_order"></div>
...
//! Check Modal AttachFile'
  $(document).ready(function() {
    $('.BttAttachTxT').click(function() {
      var ids = $(this).attr('data-id');
      var emps = $(this).attr('data-emp');
      $("#ItemReqID").val(ids);
      $("#ItemEmpID").val(emps);
      $('#AttachmentTxTModal').modal('show');
      var ItemReqNo = $('#ItemReqID').val();
      var ItemEmp = $('#ItemEmpID').val();
      if (ItemReqNo != '' && ItemEmp != '') {
        $.ajax({
          url: "return/return_closing_attachtxt.php",
          method: "POST",
          data: {
            Item_ReqNo: ItemReqNo,
            Item_Emp: ItemEmp
