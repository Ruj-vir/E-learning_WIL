...
if (isset($_GET["ItemReqNo"]) && trim($_GET["ItemReqNo"]) != NULL) {
    ...
    $TrainSql = "SELECT DISTINCT TrainRecNo FROM TrainRecHdr WHERE (TrainRecNo = '$ItemReqNo') AND (CreateBy = '$SesUserID') ";
    $ListIParams = array();
    $ListIOptions =  array("Scrollable" => SQLSRV_CURSOR_KEYSET);
    $ListIStmt = sqlsrv_query($connEducation, $TrainSql, $ListIParams, $ListIOptions);
    ...
    if ($ListIRow > 0) {
        ...
        FROM ReqInfo 
        WHERE (ReqNo = '$ItemReqNo') AND (ReqType = 4) ";
        
        $ListObj = sqlsrv_query($connRequest, $ListSql);
        $ListResult = sqlsrv_fetch_array($ListObj, SQLSRV_FETCH_ASSOC);

        $CountSql = "SELECT COUNT(ReqNo) AS ReqEmployee FROM ReqInfo WHERE (ReqNo = '$ItemReqNo') AND (ReqType = 4) ";
