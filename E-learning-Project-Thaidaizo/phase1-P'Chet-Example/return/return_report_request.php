...
<div class="table-wrapper-scroll-y-01 my-custom-scrollbar-01">
  <table class="table table-hover nowrap text-truncate" style="width:100%">
   <thead>
     <tr>
       <th scope="col">
         <input type="hidden" id="select_list" name="select_list">
       </th>
  ...
  $sql = "SELECT dbo.ReqInfo.ReqNo, dbo.ReqInfo.EmployeeID, dbo.ReqInfo.Status, dbo.ReqUser.EmpUserName, 
  dbo.ReqUser.EmpUserSurname, dbo.ReqUser.EmpUserSection, dbo.ReqUser.EmpUserDepartment

   FROM dbo.ReqInfo INNER JOIN dbo.ReqUser ON dbo.ReqInfo.EmployeeID = dbo.ReqUser.EmpUserID
   WHERE (dbo.ReqInfo.ReqNo = '$ItemReqNo') AND (dbo.ReqInfo.ReqIssuer = '$SesUserID') AND (dbo.ReqInfo.ReqType = 4) ";
  ...
  while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
    ...
    $output .= '<tr>
        <th scope="row">
          <input type="'.(($row["Status"] == 1) ? "checkbox" : "hidden").'" class="item_checkbox pointer" value="' . $row["ReqNo"] . '" name="listStaff1[]">
          <input type="checkbox" class="item_checkbox10 pointer" value="' . $row["EmployeeID"] . '" name="listStaff2[]" hidden="hidden">
        </th>
        <th class="text-secondary">' . $i . '.</th>
