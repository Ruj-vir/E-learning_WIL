...
if (isset($_POST["Item_ReqNo"])) {
  ...
  <div class="table-wrapper-scroll-y-01 my-custom-scrollbar-01">
    ...
    $sql = "SELECT dbo.ReqInfo.ReqNo, dbo.ReqInfo.EmployeeID, dbo.ReqUser.EmpUserName, 
    dbo.ReqUser.EmpUserSurname, dbo.ReqUser.EmpUserSection, dbo.ReqUser.EmpUserDepartment, dbo.ReqInfo.Status 
    ...
    while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
      ...
      $output .= '<tr>
        <th scope="row">
          <input type="checkbox" class="item_checkbox '.(($row["Status"] == 2) ? "pointer" : "").'" value="' . $row["ReqNo"] . '" name="listStaff1[]" '.(($row["Status"] == 2) ? "" : "disabled").'>
