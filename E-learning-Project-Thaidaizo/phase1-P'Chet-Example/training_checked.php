      ...
      <div class="card shadow">
        <div class="card-header text-uppercase">
          <div class="row justify-content-between">
            <div class="col">
              <i class="fas fa-clipboard-check"></i> Checked
            </div>
            <div class="col text-right">
              <a href="training_check.php">Check <i class="fas fa-arrow-alt-circle-right"></i></a>
            </div>
          </div>
        </div>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header text-greenblue">
        <h5 class="modal-title" id="reque_title">Checked</h5>
  ...
  $(document).ready(function() {
    $('.DateStart, .DateEnd').on('keyup change', function() {
      ...
      if (((DateFrom != "" && DateTo != "") && (DateFrom <= DateTo))) { //alert(DateTo);
        $.ajax({
          url: "return/return_searchchecked.php",
          method: "POST",
          data: {
            DateFrom: DateFrom,
            DateTo: DateTo
