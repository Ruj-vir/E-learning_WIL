...
<?php
	...
	$stmt10 = "SELECT DISTINCT ReqChecker FROM ReqInfo 
	WHERE (ReqNo=$ItemID) 
	...
	while($result10 = sqlsrv_fetch_array($query10, SQLSRV_FETCH_ASSOC)){
		...
		AND (ReqChecker='".$result02["ReqChecker"]."') 
		AND (UpdateBy='$SesUserID') 
		AND (UpdateDate IS NOT NULL)
		AND (ReqType = 4) 
		AND (Status = 0)";
	...
	WHERE (EmpUserID='".$result10["ReqChecker"]."')";
	...
	$strSubject = "Reject a request to Check (from Approver).";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   AND (UpdateBy='$SesUserID') 
		   AND (UpdateDate IS NOT NULL) 
		   AND (ReqChecker='".$result10["ReqChecker"]."') 
		   AND (ReqFleg = 'A1')
		   AND (Status=0) 
		   AND (ReqType = 4)";
