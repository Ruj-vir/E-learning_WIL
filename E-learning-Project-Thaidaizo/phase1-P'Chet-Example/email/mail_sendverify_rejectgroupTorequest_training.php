...
for($i=0;$i<count($_POST["CheckStaff"]);$i++) {
	...
	$objResulty11 = sqlsrv_fetch_array($objQueryer11,SQLSRV_FETCH_ASSOC);
		...
		WHERE (ReqNo='".$_POST["CheckStaff"][$i]."') 
		AND (ReqIssuer='".$result02["ReqIssuer"]."') 
	...
	$strSubject = "Reject the request to the requester (from Verify).";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   WHERE (ReqNo='$ItemID') 
		   AND (ReqIssuer = '".$objResulty11["ReqIssuer"]."') 
		   AND (ReqApprover='$SesUserID') 
		   AND (ReqApproveDate IS NOT NULL)
		   AND (ReqFleg = 'V1') 
		   AND (ReqType = 4) 
...
$strMessage .= "</table>
	...
	$flgSendRequest = @mail($strTo,$strSubject,$strMessage,$strHeader);  // @ = No Show Error // #3D5AFE
}
	if($flgSendRequest){
		//echo "Email Sending.";
		...
		echo "<script type=text/javascript>alert('Error!! Email Can Not Send to Request.');window.top.window.TrainingResult('0');</script>";
		exit();
    }
