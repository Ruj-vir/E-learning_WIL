...
<?php
	...
	$stmt10 = "SELECT DISTINCT ReqIssuer FROM ReqInfo 
	WHERE (ReqNo=$ItemID) 
	...
	while($result10 = sqlsrv_fetch_array($query10, SQLSRV_FETCH_ASSOC)){
		...
		//!
		/*$sql = "SELECT EmployeeID,ReqFleg FROM ReqInfo WHERE (ReqNo=$ItemID) 
	...
	$stmt03 = "SELECT EmpUserID,EmpUserName,EmpUserSurname,EmpUserEmail FROM ReqUser WHERE (EmpUserID='".$result10["ReqIssuer"]."')";
	...
	WHERE (ReqNo = $ItemID) AND (Status = 0)";
	...
	$strSubject = "Reject a request to Requester (from Approver).";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   AND (UpdateBy='$SesUserID') 
		   AND (UpdateDate IS NOT NULL) 
		   AND (ReqIssuer='".$result10["ReqIssuer"]."') 
		   AND (ReqFleg = 'A1')
		   AND (Status=0) 
		   AND (ReqType = 4)";
