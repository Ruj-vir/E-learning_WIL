...
if (isset($_POST["SubmitCheckedGroup"]) == '1') {
	...
	foreach ($_POST["CheckStaff"] as $iNum => $ItemKey) {
		$SQLCheck = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo=$ItemKey) AND (ReqApprover='$SesUserID') AND (Status=4)";
	...
	$InChecker = substr($CheckerMark, 0, -1);

	//! เช็คผู้ตรวจสอบ \\
	$AuthorCheckSQL = "SELECT COUNT(EmpUserID) AS NumAuthorCheck FROM ReqUser WHERE EmpUserID IN ($InChecker) AND (EduVerify IS NULL)";
	...
	$AuthorVerifySQL = "SELECT COUNT(EmpUserID) AS NumAuthorVerify FROM ReqUser WHERE EmpUserID IN ($InChecker) AND (LvApprove IS NULL)";
	...
	$AuthorApproveSQL = "SELECT COUNT(EmpUserID) AS NumAuthorApprove FROM ReqUser WHERE EmpUserID IN ($InChecker) AND (EduApprove IS NULL)";
	...
	if ($NumAuthorCheck > 0) {
		...
		for ($i = 0; $i < count($_POST["CheckStaff"]); $i++) {
			...
			$StmtEmpID = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo='$RecItemKey') AND (ReqApprover='$SesUserID') AND (Status=3)";
			...
			WHERE ReqNo = ? AND EmployeeID = ? ";
				$ApproveParams = array(4, $SesUserID, $FineApproveID, $RecItemKey, $RecTrainees);
		...
		if ($ApproveStmt === false) {
			...
			include_once "../email/mail_sendApprovegroup_training.php";
			exit();
		}
	}
}
...
if (isset($_POST["RejectCheckedGroup"]) == '3') {
	...
	for ($i = 0; $i < count($_POST["CheckStaff"]); $i++) {
		...
		if (trim($RecItemKey) != NULL) {
			...
			$EmpIDSql = "SELECT EmployeeID,UserDefine1 FROM ReqInfo WHERE (ReqNo = '$RecItemKey') AND (ReqApprover='$SesUserID') AND (Status=3) ";
	...
	if ($StmtReject == true) {
		// echo 1;
		include_once "../email/mail_sendapproverejectgroup_training.php";
	} else {
		//echo  ("Error Query [".$SqlReject."]");
