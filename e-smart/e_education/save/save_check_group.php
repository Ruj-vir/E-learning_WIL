...
if (isset($_POST["SubmitCheckedGroup"]) == '1') {
	...
	//! เช็คผู้ตรวจสอบ \\
	$AuthorCheckSQL = "SELECT COUNT(EmpUserID) AS NumAuthorCheck FROM ReqUser WHERE EmpUserID IN ($InChecker) AND (LvApprove IS NULL)";
	...
	$AuthorVerifySQL = "SELECT COUNT(EmpUserID) AS NumAuthorVerify FROM ReqUser WHERE EmpUserID IN ($InChecker) AND (EduVerify IS NULL)";
	...
	if ($NumAuthorCheck > 0) {
		...
		if ($ApproveStmt === false) {
			...
			include_once "../email/mail_sendcheckgroup_training.php";
			exit();
		}
	}
}
...
if (isset($_POST["RejectCheckedGroup"]) == '3') {
	...
	if ($StmtReject == true) {
		include_once "../email/mail_sendcheckrejectgroup_training.php";
	} else {
