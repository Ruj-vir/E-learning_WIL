...
if (isset($_POST["SubmitVerifyGroup"]) == '1') {
	...
	if ($NumAuthorCheck > 0) {
		...
		for ($i = 0; $i < count($_POST["CheckStaff"]); $i++) {
			...
			WHERE ReqNo = ? AND EmployeeID = ? ";
				$ApproveParams = array(3, $SesUserID, $FineApproveID, $RecItemKey, $RecTrainees);
...
if (isset($_POST["RejectCheckedGroup"]) == '3') {
	...
	if ($StmtReject == true) {
		// echo 1;
		include_once "../email/mail_sendverifyrejectgroup_training.php";
	} else {
