...
if (isset($_POST["SubmitVerifyList"]) == '1') {
	...
	if ($NumAuthorCheck > 0) {
		...
		if ($ApproveStmt === false) {
			...
			include_once "../email/mail_sendVerifylist_training.php";
			exit();
		}
	}
...
if (isset($_POST["RejectVerifyList"]) == '3') {
	...
	for ($i = 0, $a = 0; $i < count($ItemKey) && $a < count($Trainees); $i++, $a++) {
		if (isset($ItemKey[$i], $Trainees[$a])) {
			...
			$ParamsReject = array($ReasonReject, $SesUserID, 0, $RecItemKey, $RecTrainees, $SesUserID, 2);
	...
	if ($StmtReject == true) {
		// echo 1;
		include_once "../email/mail_sendverifyrejectlist_training.php";
	} else {
