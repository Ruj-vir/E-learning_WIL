...
<?php
	...
	$stmt02 = "SELECT DISTINCT ReqApprover FROM ReqInfo WHERE (ReqNo=$EmpReq) 
	...
	while($result02 = sqlsrv_fetch_array($query02, SQLSRV_FETCH_ASSOC)){
		...
		AND (ReqApprover='".$result02["ReqApprover"]."') 
		AND (ReqApprover='$SesUserID') 
		AND (ReqType = 4) AND (Status = 3)";
	...
	WHERE (EmpUserID='".$result02["ReqApprover"]."')";
	...
	WHERE (ReqNo = $EmpReq) AND (ReqType = 4)";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   if($obj){
		   $stmt00 = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo=$EmpReq) AND (ReqApprover='".$result02["ReqApprover"]."') AND (ReqApprover='$SesUserID') AND (ReqType = 4) AND (ReqFleg = 'C1') AND (Status = 4)";
...
$strMessage .= "<tr style='height: 40px; font-size: 16px;'>
            ...
            <td style='border-right: thin solid #1de9b6; border-bottom: thin solid #1de9b6; text-align: center;'><b>ตรวจสอบแล้ว</b> <span style='font-size: 14px;'>(Approved)</span></td>
           </tr>";
		   $i++;
           }}
