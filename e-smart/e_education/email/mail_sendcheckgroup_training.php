...
<body>
	...
	foreach ($_POST["CheckStaff"] as $ItemID) {
		...
		$stmt10 = "SELECT DISTINCT ReqApprover FROM ReqInfo WHERE (ReqNo=$ItemID) AND (ReqChecker='$SesUserID') AND (Status=2)";
		$query10 = sqlsrv_query($connRequest, $stmt10);
		while ($result10 = sqlsrv_fetch_array($query10, SQLSRV_FETCH_ASSOC)) {

			$stmt03 = "SELECT EmpUserID,EmpUserName,EmpUserSurname,EmpUserEmail FROM ReqUser WHERE (EmpUserID='" . $result10["ReqApprover"] . "')";
	...
	WHERE (ReqNo = $ItemID) AND (Status = 2)";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   <tr style='height: 40px; font-size: 18px; color: #343a40; background-color: #1de9b6; '>
			...
			$Queryer12 = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo=$ItemID) AND (ReqChecker='$SesUserID') AND (ReqApprover='" . $result10["ReqApprover"] . "') AND (Status=2)";
