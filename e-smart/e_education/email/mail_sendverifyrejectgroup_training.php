...
for($i=0;$i<count($_POST["CheckStaff"]);$i++) {
    ...
    $Queryer11 = "SELECT ReqDate,TrnTime,ReqDay,ReqHour,ReqSumTime,ReqRemark,ReqIssuer FROM ReqInfo 
	WHERE (ReqNo='".$_POST["CheckStaff"][$i]."') AND (ReqApprover='$SesUserID') AND (Status=0)";
    $objQueryer11 = sqlsrv_query($connRequest, $Queryer11);
	$objResulty11 = sqlsrv_fetch_array($objQueryer11,SQLSRV_FETCH_ASSOC);

	$Queryer14 = "SELECT EmpUserName,EmpUserSurname,EmpUserEmail FROM ReqUser 
	WHERE (EmpUserID='".$objResulty11["ReqIssuer"]."')";
	$objQueryer14 = sqlsrv_query($connRequest, $Queryer14);
	$objResulty14 = sqlsrv_fetch_array($objQueryer14,SQLSRV_FETCH_ASSOC);
	$OtChecker = $objResulty14["EmpUserName"]."\n".$objResulty14["EmpUserSurname"];
	...
	$strSubject = "Inform to Reject request";
	...
	<div line-height: 4px;>
		<div style='font-size: 20px;'>เนื่องด้วยผู้บังคับบัญชาของท่าน ได้<span style='text-decoration: underline;'>ปฏิเสธ</span>ได้ทำการร้องขอการฝึกอบรม ตามรายละเอียด ดังนี้</div>
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   $Queryer12 = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo='".$_POST["CheckStaff"][$i]."') AND (ReqIssuer = '".$objResulty11["ReqIssuer"]."') AND (ReqApprover='$SesUserID') AND (ReqType = 4) AND (Status=0)";
...
$strMessage .= "<tr style='height: 40px; font-size: 16px;'>
            ...
            <td style='border-right: thin solid #1de9b6; border-bottom: thin solid; text-align: center;'><span style='color: #ff1744;'><b>ไม่อนุมัติ</b> <span style='font-size: 14px;'>(Rejected)</span></span></td>
           </tr>";
		   $i++;
           }
...
$strMessage .= "</table>
	...
	<br/><br/>
	<div line-height: 4px;>
	<div style='font-size: 20px;'>เรียนมาเพื่อทราบ</div>
	<div style='font-size: 18px;'>Best Regards.</div>
	</div>
