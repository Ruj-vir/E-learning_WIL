...
<?php
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   if($obj){
		   $stmt00 = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo=$EmpReq) AND (ReqApprover='".$result02["ReqApprover"]."') AND (ReqChecker='$SesUserID') AND (ReqType = 4) AND (ReqFleg = 'C1') AND (Status = 2)";
...
$strMessage .= "<tr style='height: 40px; font-size: 16px;'>
            ...
            <td style='border-right: thin solid #1de9b6; border-bottom: thin solid #1de9b6; text-align: center;'><b>ตรวจสอบแล้ว</b> <span style='font-size: 14px;'>(Checked)</span></td>
           </tr>";
		   $i++;
           }}
