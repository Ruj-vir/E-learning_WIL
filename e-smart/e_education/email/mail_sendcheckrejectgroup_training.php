...
for($i=0;$i<count($_POST["CheckStaff"]);$i++) {
	...
	WHERE (ReqNo='".$_POST["CheckStaff"][$i]."') AND (ReqChecker='$SesUserID') AND (Status=0)";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   $Queryer12 = "SELECT EmployeeID FROM ReqInfo WHERE (ReqNo='".$_POST["CheckStaff"][$i]."') AND (ReqIssuer = '".$objResulty11["ReqIssuer"]."') AND (ReqChecker='$SesUserID') AND (ReqType = 4) AND (Status=0)";
