...
<?php
    ...
    $Queryer12 = "SELECT DISTINCT ReqIssuer FROM ReqInfo WHERE (ReqNo=$EmpReq) 
    ...
    while($objResulty12 = sqlsrv_fetch_array($objQueryer12,SQLSRV_FETCH_ASSOC)){
		...
		$sql = "SELECT EmployeeID,ReqFleg FROM ReqInfo WHERE (ReqNo=$EmpReq) 
		...
		while($CR = sqlsrv_fetch_array($oop, SQLSRV_FETCH_ASSOC)){
			...
			if($str == NULL){
				...
				$vowels = array("C");
				$cutstr = str_replace($vowels,'', $str);
				$SumCk = $cutstr + 1;
				$Ckreq = 'C'.$SumCk;
			}
			$str= "UPDATE ReqInfo SET ReqFleg = '$Ckreq' WHERE (ReqNo=$EmpReq) AND (EmployeeID = $Emp)";
	...
	WHERE (ReqNo = $EmpReq) AND (ReqApprover='$SesUserID') AND (Status=0)";
	...
	<br/>
		...
		<table style='width: 100%;'cellspacing='0' cellpadding='0'>
		   ...
		   if($obj){
            $Queryer6 = "SELECT EmployeeID FROM ReqInfo WHERE(ReqNo=$EmpReq) AND (ReqApprover = '$SesUserID') AND (ReqIssuer = '".$objResulty12["ReqIssuer"]."') AND (ReqType = 4) AND (ReqFleg = 'C1') AND (Status = 0)";
