        ...
        <div class="card-header text-uppercase">
          <div class="row justify-content-between">
            ...
            <div class="col text-right">
              <a href="training_checked.php">Checked <i class="fas fa-arrow-alt-circle-right"></i></a>
            </div>
          </div>
        </div>

        <div class="card-body">
          <form id="formCheck" name="formCheck" class="form" action="save/save_approve_group.php" autocomplete="off" method="POST" target="iframe_check_ReqGroup">
...
WHERE (ReqInfo.ReqType = 4) AND (ReqInfo.ReqChecker = '$SesUserID') AND (ReqInfo.Status = 3)
ORDER BY ReqInfo.ReqNo";
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="form-group">
                      ...
                      <textarea class="form-control" id="RejectForm" name="txtReject" rows="3" placeholder="Enter.."></textarea>
                    </div>

                  </div>
                  <div class="modal-footer">
                    <button type="submit" name="RejectCheckedGroup" value="3" class="btn btn-dark" onclick="return BttCancel();">Confirm</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="card-footer border-light bg-white cart-panel-foo-fix">
              <div class="d-flex mt-3">
                ...
                <div class="mr-1"><button type="submit" name="SubmitCheckedGroup" value="1" class="btn btn-success" onclick="return BttSubmitCheck();"><i class="fa fa-check-circle"></i> Check</button></div>
                <div><button type="button" class="btn btn-dark" onclick="return BttSubmitReject();"><i class="fa fa-times-circle"></i> Reject</button></div>
              </div>
            </div>
...
<!-- Modal Check -->
<form id="frmrequest" name="frmrequest" class="form" action="save/save_approve_list.php" autocomplete="off" method="POST" target="iframe_check_Reqlist">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        -->
          ...
          <button type="submit" name="SubmitCheckedList" value="1" class="btn btn-success" onclick="return BttSubmitCheckList();"><i class="fa fa-check-circle"></i>&nbsp; Check</button>
          <button type="button" class="btn btn-dark" data-toggle="modal" data-target="#Cancel_list" onclick="return BttSubmitRejectList();"><i class="fa fa-times-circle"></i>&nbsp; Reject</button>
            ...
            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="form-group">
                    <label for="inputReason">Reason for cancellation</label>
                    <textarea class="form-control" id="RejectList" name="txtReject" rows="3" placeholder="Enter.."></textarea>
                  </div>
                </div>
                <div class="modal-footer">
                  <button type="submit" name="RejectCheckedList" value="3" class="btn btn-dark" onclick="return Bttlist();">Confirm</button>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="modal-dialog modal-confirm">
        <div class="modal-content">
            <div class="modal-header">
                <div class="icon-box">
                    <i class="far fa-check-circle" style="font-size: 52px;"></i>
                </div>
...
<script type="text/javascript">
    $('#ModalSuccess').on('hidden.bs.modal', function() {
        window.location.href = 'training_check.php';
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    $('.BttLookModalCheck').click(function() {
      ...
      var Item_ReqNo = $('#txtReqNo').val();
      if (Item_ReqNo != '') {
        $.ajax({
          url: "return/return_approve.php",
          method: "POST",
          data: {
  ...
  $(document).ready(function() {
    $('.BttChecked').click(function() {
      ...
      var Item_Detail = $('#txtNoDetail').val();
      if (Item_Detail != '') {
        $.ajax({
  ...
  $(document).ready(function() {
    $('.DateStart, .DateEnd').on('keyup change', function() {
      ...
      if (((DateFrom != "" && DateTo != "") && (DateFrom <= DateTo))) { //alert(DateTo);
        $.ajax({
          url: "return/return_searchchecked_user.php",
          method: "POST",
          data: {
            DateFrom: DateFrom,
            DateTo: DateTo
...
<script type="text/javascript">
  ...
  /*function BttSubmitRejectList() {
     ...
     else if ($('.item_checkbox').is(':checked')){
