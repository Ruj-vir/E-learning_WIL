<?php
    ...
    FROM dbo.Cnt_TrnDetail INNER JOIN HRSystem.dbo.eEmployee ON dbo.Cnt_TrnDetail.EmpID = HRSystem.dbo.eEmployee.sEmpID
		WHERE (dbo.Cnt_TrnDetail.Status = 0 OR dbo.Cnt_TrnDetail.Status = 1 OR dbo.Cnt_TrnDetail.Status = 3) AND (dbo.Cnt_TrnDetail.TrnDate = '$Datelimit') AND (dbo.Cnt_TrnDetail.MakeBy = 'Canteen TDP') ORDER BY dbo.Cnt_TrnDetail.TrnTime DESC";
  ...
  $stmt10 = "SELECT COUNT(EmpID) AS Summer FROM Cnt_TrnDetail WHERE (Status = 1 OR Status = 3) AND (MakeBy = 'Canteen TDP') AND (TrnTime BETWEEN '$TimeFrom10' AND '$TimeTo10')";
  ...
  $stmt20 = "SELECT COUNT(EmpID) AS Summer FROM Cnt_TrnDetail WHERE (Status = 1 OR Status = 3) AND (MakeBy = 'Canteen TDP') AND (TrnTime BETWEEN '".$TimeFrom20."' AND '$TimeTo20')";
  ...
  $stmt30 = "SELECT COUNT(EmpID) AS Summer FROM Cnt_TrnDetail WHERE (Status = 1 OR Status = 3) AND (MakeBy = 'Canteen TDP') AND (TrnTime BETWEEN '".$TimeFrom30."' AND '$TimeTo30')";
...
<div class='post-module border rounded-sm mt-2'>
	...
	<div class='post-content'>
		<div class='category bg-secondary'><span class="">standby mode</span></div>
