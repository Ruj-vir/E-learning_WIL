...
<div class="container">
	<div class="card">
		<div class="card-header h3">
			<strong>DAILY <span class="badge badge-warning">TDA</span></strong>
		</div>
		<div class="card-body">
					...
					<table id="canteen_report" class="table table-hover nowrap" style="width:100%">
						...
						<tbody class="tbody">
							...
							$stmt = "SELECT dbo.Cnt_TrnDetail.TrnNo, dbo.Cnt_TrnDetail.EmpID, HRSystem.dbo.eEmployee.sEmpFirstName, HRSystem.dbo.eEmployee.sEmpLastName, HRSystem.dbo.eEmployee.sEmpEngFirstName,
							   ...
							   FROM dbo.Cnt_TrnDetail INNER JOIN HRSystem.dbo.eEmployee ON dbo.Cnt_TrnDetail.EmpID = HRSystem.dbo.eEmployee.sEmpID WHERE (dbo.Cnt_TrnDetail.Status = 3) AND (dbo.Cnt_TrnDetail.TrnDate = '$DateStart') AND (dbo.Cnt_TrnDetail.MakeBy = 'Canteen TDA') ORDER BY dbo.Cnt_TrnDetail.TrnTime DESC";
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#From, #to').on('keyup change', function() {
			...
			if ((From != '' && to != '') && (From <= to)) {
				$.ajax({
					url: "return/return_reportday_tda.php",
					method: "POST",
					data: {
						From: From,
						to: to
